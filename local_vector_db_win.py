...
# Add this: For Windows console close event handling
if platform.system() == "Windows":
    import win32api
    import win32con
    import ctypes

    def handle_windows_shutdown(sig):
    ...
    # Set console ctrl handler for Windows
    def setup_windows_handlers():
        ...
        def console_ctrl_handler(ctrl_type):
            ...
            if ctrl_type in (win32con.CTRL_C_EVENT, win32con.CTRL_BREAK_EVENT,
                ...
                handle_windows_shutdown(ctrl_type)
                return True
            return False

        # Register the handler
        win32api.SetConsoleCtrlHandler(console_ctrl_handler, True)
        print("Windows shutdown handlers registered")

def increase_file_limit(target_limit=65535):
    """Set the file descriptor limit to the target_limit if possible."""
    # Skip on Windows as resource module is Unix-specific
    if platform.system() == "Windows":
        print("File descriptor limit adjustment skipped on Windows.")
        return
...
class ChromaDBManager:
    ...
    def cleanup_db(self, db_name: str):
            ...
            if db_name in self.dbs:
                try:
                    # Instead of modifying properties directly, just remove the reference
                    # Let Python's garbage collector handle the cleanup
                    db = self.dbs.pop(db_name, None)
    ...
    def cleanup_all(self):
        ...
        for db_name in list(self.dbs.keys()):
...
db_paths = glob(os.path.join(parent_dir, "knowledge*", "knowledge_base*"))

# If no paths found with the above pattern, try Windows-specific pattern
if not db_paths:
    db_paths = glob(os.path.join('..', 'knowledge*', 'knowledge_base*'))
...
def main():
    ...
    try:
        # Register Windows-specific handlers if on Windows
        if platform.system() == "Windows":
